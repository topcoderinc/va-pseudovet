...
def build_problem(lines, patient, collection='icd'):
    ...
    if collection=='icd':
        return {'code':lines[index][:8].strip(), 'name':icd_lines[index][8:].strip(), 'resolved': resolved, 'onset': onset, 'resolved_on': resolved_on}
    else:
        # war collection
        data = lines[index].split(',')
        return {'code':data[1].strip('"'), 'name':data[0].strip('"'), 'resolved': resolved, 'onset': onset, 'resolved_on': resolved_on}
        
def random_icd_problem(params):
    '''
    Method defined in datasource.json called from get_rand_value() in pseudo-vets.py
    ...
    if icd_lines is None:
        ...
        try:
            ...
            icd_lines = [line.rstrip('\n') for line in open('./datasources/ICD-10/icd10cm_codes_2018.txt')]
        ...
        if war_lines is None:
            ...
            for war in ['gulf_war','korean_conflict','vietnam_war','world_war_ii']:
                try:
                    ...
                    lines = [line.rstrip('\n') for line in open('./datasources/morbidity_{0}.csv'.format(war))]
                    logger.info('Loaded {0} records from morbidity_{1}.csv'.format(len(lines), war))
                    war_lines[war]=lines

                except Exception as e:
                    logger.error('Could not open war morbidity datasource. Error: %s' % (e) )
                    exit (1)
